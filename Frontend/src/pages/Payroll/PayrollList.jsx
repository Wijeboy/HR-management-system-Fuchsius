...
const PayrollList = () => {
  ...
  const loadPeriods = useCallback(async () => {
    try {
      ...
      const nextPeriods = ['All Periods', ...new Set(list.map((record) => record.period))];
  ...
  const filteredCountLabel = useMemo(() => {
    ...
    if (records.length === 0) return 'No payroll records found';
    return `Showing ${records.length} payroll records`;
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
              ...
              {records.map((record) => (
                <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-700">
                    ...
                    <p className="text-xs text-gray-500">
                      Payroll ID: {record.id} · Paid: {record.paymentDate}
                    </p>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-700">
                    <p>{record.attendanceDays} days present</p>
                    <p className="text-xs text-gray-500">{record.leaveDays} leave days</p>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-700">
                    ...
                    <p className="mt-1 font-bold text-indigo-700">Net {formatCurrency(record.net)}</p>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="inline-flex items-center gap-2">
                      ...
                      <Link
                        to={`/payroll/generate?employee=${record.employeeId}`}
        ...
        <div className="flex items-center justify-between border-t border-gray-200 px-6 py-4">
          <p className="text-sm text-gray-600">{filteredCountLabel}</p>
